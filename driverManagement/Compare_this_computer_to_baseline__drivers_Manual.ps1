# Compares Device Driver information from this computer to baseline Win32_pnpsigneddriver XML-file
# Good for making sure you install right drivers comparing to baseline which you have made earlier
#
# You can make baseline XML-file on workstation in Powershell with command:
...
#
# Or use Export_drivers.ps1 which will make xml file automatically
...
# Test are there enough parameters (1)
if ($psboundparameters.Count -ne 1) {
    Write-Host "Give parameter to 1 XML-files to compare" 
...
[System.Collections.ArrayList]$computer2 = Get-WmiObject Win32_pnpsigneddriver | Where-object {$_.DeviceClass -ne "LEGACYDRIVER" -and ![string]::IsNullOrEmpty($_.DeviceClass)} | Select-Object DeviceID, DeviceName, DeviceClass, Manufacturer, DriverProviderName, DriverVersion|sort-object DeviceName
